...
def export_text_config(test_config):
...
# For the first n epochs, train with a lower learning rate (lr_init*0.1). This can be helpful if the model immediately explodes.
pre_low_lr      = 0
...
ndim_x     = 7  # seven parameters for band structure
ndim_pad_x = 2

ndim_y     = 8 # esn with a polyfit of degree 5
ndim_z     = 1
ndim_pad_zy = 0
...
add_pad_noise = {}
add_y_noise = {}""".format(test_path,logfile,data_path,device,batch_size,hidden_layer_sizes,N_blocks,exponent_clamping,dropout_perc,batch_norm,use_permutation,ndim_x,ndim_pad_x,ndim_y,ndim_z,ndim_pad_zy,add_pad_noise,add_y_noise)

export_text_config(test_config)
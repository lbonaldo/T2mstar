# julia visualization.jl /mnt/scratch/bonal1lCMICH/inverse/spline/results/Jan-26-2022/16-15-17/test

# if length(ARGS) == 0
#     println("[FATAL]\t Missing model path. Quit.")
#     exit(-1) 
# end
...
test_name = "/mnt/scratch/bonal1lCMICH/inverse/spline/results/Jan-26-2022/17-21-33/test"
#test_name = ARGS[1]

plots_path = joinpath(test_name, "plots")
if !isdir(plots_path) # if there is not a results folder -> create it
    mkdir(plots_path)
end
...
for (i,coeff) in enumerate(coeff_name)
    ...
    scatter!(axmain, res[:, coeff*"_pred"], res[:, coeff*"_true"], color = (c, 0.2), label = coeff)

...
function f_export(idx::Int64)
    filename = "coeff_$idx.csv"
    data = Matrix{Float64}(CSV.read("./model/test/results/test_2_xcoeff/data/"*filename, DataFrame; delim=",", header=0))
    ...
    #display(fig)
    savefig("model/test/results/test_2_xcoeff/plots/jplot_$idx.png")
end
